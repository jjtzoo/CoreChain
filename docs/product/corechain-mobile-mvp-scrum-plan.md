...
- **Not yet verified on the phone:** a new edit reaching the server as an update, a record made on the server appearing on the phone, the depth shortcuts, and sign-out/sign-in on the same phone. Still to do in S4: use the device-issued sample-number blocks (E6-4, phone side) and a "Send feedback" entry on Home. Production PowerSync instance: added before the field test.
