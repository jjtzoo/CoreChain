...
- **Still to do:** the data-entry forms (new box, run, interval, sample) still use the old layout with the new colours and buttons; a bottom tab bar (Home, Photo, Samples, Export) is designed but not built; the bagging action; the same care for empty states and the settings, codes and export screens.
