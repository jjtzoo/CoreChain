...
- **PowerSync spike, verified as far as possible without a device:**
  ...
  - SQLCipher is enabled via `"op-sqlite": { "sqlcipher": true }` in the **root** `package.json` (required in a monorepo, per op-sqlite's docs) — done.
  ...
  - **Not yet done, because it needs a device/emulator:** actually opening an encrypted database, writing and reading a row, and confirming it survives an app kill. This is now the first task of Sprint 1's E8-1 story, and E8-1's existing acceptance criteria already cover it.
...
**Still open before Sprint 1 can be verified end-to-end: get Android Studio (or a physical Android device with USB debugging) set up on a development machine.**

### S1 outcome (2026-09-19)

All seven stories (E8-1, E1-1, E1-2, E2-1 through E2-4) are coded, typechecked, linted and structurally bundle-verified — the same Android-tooling gap from Sprint 0 still blocks the one thing that would make them formally Done: **running on a real device**. Specifics:
...
| SQLCipher working with PowerSync           | Resolved in S0          | —                     | Confirmed: `@op-engineering/op-sqlite` (PowerSync's built-in storage adapter) bundles SQLCipher; enabled via `"op-sqlite": {"sqlcipher": true}` in the root `package.json`. Requires `expo prebuild` — **op-sqlite cannot run in Expo Go.** On-device read/write is still unverified; that's Sprint 1's E8-1. |
