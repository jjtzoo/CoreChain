...
- **Verified in code, not yet on the phone.** Typecheck, lint, the full test suite (`appVersion.test.ts`'s new cases plus the updated `syncStatus.test.ts` cases) and the web production build all pass, and `npx expo export --platform android` bundles the app cleanly with the new code path. Not checked: the actual message on the Account screen, or that syncing genuinely stops and resumes, on the real test phone — that needs a build where `MIN_APP_VERSION` is temporarily set above `0.1.0` to force the outdated state, the same kind of deliberate real-device check E10-3's crash was verified with.
