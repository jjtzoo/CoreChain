...
| Sections, 3D interpretation and modelling | Specialist tools do this; CoreChain exports to them. The collar map (E15) and the project manager's 3D evidence view on the web (D18, E18) show recorded evidence only. |
...
| D18 | **The resident / project manager gets a 3D evidence view of each project, on the web** (2026-09-27, owner) | It shows the recorded evidence in place: where each hole runs, which intervals were sampled, and what the laboratory returned, coloured by element. It is not interpretation (no interpolated surfaces or grade shells); that stays with specialist tools, as the operations document says. Traces follow the planned azimuth and dip, because there is no downhole survey, and depths are below the collar, because collar elevation is not recorded; the view says both. The phone keeps the plan view (the collar map); a section view on the phone is a later option. Who else should see it waits on the roles conversation for the pilot (E18, "To settle"). |
...
**E17 — Device limits and approval by the project manager (web and phone, about 2 to 3 sessions).** Backlogged by the owner on 2026-09-24 in favour of more important work. Today any account can sign in on any number of phones, and each phone registers itself on first sync with no approval (`apps/web/lib/devices.ts`). A device can be blocked (`revoked_at`: the server then refuses its uploads and new sample-number blocks), but no screen sets it; only the database can. Proposed:

- **Limits per account:** a field geologist has at least 1 and at most 2 active phones (a main phone and a spare); a resident / project manager has at least 1 and at most 3. Web sign-ins are not counted, because a browser holds no offline data or sample-number blocks; a "Signed-in browsers" list with "Sign out everywhere" covers them instead.
- **Approval:** a new phone registers as "Waiting for approval" and gets no team data or sample numbers until the project manager approves it under Team overview, Devices. Approve, Refuse, and Remove (which uses the existing `revoked_at` block, for a lost phone). The admin can act on any team. Removing a phone never deletes what it already synced; custody stays append-only.
- **Needs:** a migration (approval status on devices, a per-team limit), which the owner applies; the web approve and remove screens; a phone screen "Waiting for your project manager to approve this phone" (new APK); and a sync-rule change so an unapproved phone downloads nothing.
- **To decide before building:** the limits above, and whether a phone may keep logging offline while it waits for approval. Whether a lead for one rig (not the project manager) should approve phones waits on the roles conversation in E18.

**E18 — Project page and 3D evidence view for the project manager (web only, about 3 to 4 sessions).** Chosen by the owner on 2026-09-27 (D18). Today the manager's `/team` overview lists every hole in the team, across projects, and there is no page for one project. No new APK, no migration and no sync change: it reads what is already on the server.

- **E18-1 — Project page.** `/team/projects/[projectId]`, for the resident / project manager, limited to their own team's projects. The header gives the project, commodity and location, the hole count, and how many sampled intervals have laboratory results and how many are still waiting. Project names on the team overview open it; each hole's page gets "See in 3D", which opens it with that hole selected.
- **E18-2 — 3D evidence view coloured by element.** The project's holes along their planned azimuth and dip from their collars, with each primary sample's interval coloured by the chosen element on one scale for the project. Below detection and "waiting for results" are shown differently from a measured value. Standards and blanks have no depth and are not drawn; field duplicates repeat their parent's interval and are not drawn twice. Hover shows the sample number, depth and value; selecting a hole lists its samples and links to the hole page for the log, custody and photos. The view states that traces are the planned direction (no downhole survey) and depths are below the collar. The geometry and colour scale are pure functions in `packages/domain/src/projectView.ts`, with tests, reusing the collar map's `toLocalMetres` and `traceOffset`. The 3D library (three.js with React Three Fiber) loads only on this page.
- **Mockup first** (`docs/product/mockups/project-3d-view.html`), approved by the owner before building.
- **Not in E18:** colouring by rock type, a cut-off grade slider, a section view on the phone, and anything interpretive.
- **Outcome (2026-09-27): built, not yet tried with a signed-in account.** Mockup approved by the owner, then built on branch `claude/e18-project-3d-view`:
  - **Domain:** `projectView.ts` and 28 tests.
    - Traces start from the collar map's centre, so the two views line up. A hole with no dip, or angled with no azimuth, is drawn vertical and flagged.
    - The latest entry per sample and element wins. ppb, ppm, g/t and % convert to the element's usual unit; other units are shown as "can't be plotted".
    - A sample the laboratory returned without this element is "not analysed", not "waiting".
    - The default element is the most sampled, then the one with the most results above detection.
    - The depth-stretch choices follow the project's shape. Deep holes close together start at 1×; shallow holes kilometres apart start stretched.
  - **Web:**
    - The page is at `/team/projects/[projectId]`. A project from another team, or an id that isn't a UUID, gives not found; signed out, it redirects to login.
    - The view is in `components/corechain/project-view/`. The team overview's hole detail links the project name to the page. The hole page links its project and has "See in 3D" when the hole has a collar.
    - three.js, React Three Fiber and drei (about 900 KB before compression) are in async chunks, loaded only when the view opens. The project page's first load is 117 kB, level with the other team pages.
    - Changing the depth stretch re-frames the view. Opening a hole from its page flies to it.
    - If WebGL is off, the view says so. It fits a phone width with no sideways scroll.
  - **Checked:** tests (web 270, domain 521), typecheck, lint and build all pass. The view was driven in a browser on a local-only preview page (since removed) with the mockup's illustrative Cordillera data: Cu and Au, below detection, waiting for results, a hole with no collar, `?hole=`, hover and depth stretch.
  - **Not yet checked:** the page itself signed in as a project manager on the Alberta and Cordillera demo projects, and a geologist or another team's manager being refused. The only accounts on hand are the pilot testers' own. The Cordillera demo has no laboratory results (its samples are left unbagged), so on that project it shows the traces with every sample waiting.
- **To settle: roles for the pilot.** Raised by the owner while choosing E18; decide before E17 and before widening who sees this view.
  - The owner describes a lead for one rig who follows that rig's current and previous holes. The operations document's nearest match is the project geologist, or the drilling contractor's drill supervisor. Is that person a CoreChain account in the pilot, and do they need their own tier?
...
- **First kept patch on 0.1.1 (2026-09-26).** Account now ends with a version line: "CoreChain Field 0.1.1", or "CoreChain Field 0.1.1 · update of <date>" once a patch is running, so a tester can say which patch they have (PR #32). The owner published it from main with `eas update --channel production` (update group `645629ba`). On the test phone (Infinix, release APK 0.1.1 built the same day) the Account screen read "CoreChain Field 0.1.1 · update of Sep 26, 2026" after two restarts. Testers on the 0.1.0 APK do not receive 0.1.1 patches; they need the 0.1.1 APK first.
