...
| E2-4 list                   | Row shows status, depth and % logged. **Search by hole ID not yet exercised on the device.**                                                                                                                                                                                          |
| E1-2 project settings       | **Not yet exercised on the device.**                                                                                                                                                                                                                                                  |

- **Not yet verified:** airplane-mode operation as an explicit test (the app makes no network calls today, so this is expected to pass, but it should be demonstrated for the Sprint 1 demo), project settings (E1-2), and hole-ID search.
...
- **UI polish found on the device, not yet fixed:** the selected chip (status, coordinate system) is only marginally lighter than the unselected ones and will be hard to read in sunlight; the "New project" button text is tight against its edges; the "Use GPS" control looks like plain text rather than a button.
