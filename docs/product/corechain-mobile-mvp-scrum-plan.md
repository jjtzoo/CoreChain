...
- **Sample project and promo material.** `npm run sample:seed -- <email>` loads the Alberta sample project (official AGS DIG 2024-0022 holes and intervals; illustrative boxes, runs and samples) into an account. Screenshots and a LinkedIn draft are in `docs/promo/`.
- **Not yet verified on the phone:** the depth shortcuts, and sign-out then sign-in on the same phone. Still to do in S4: nothing else. Production PowerSync instance: added before the field test (the free plan includes two instances; it switches projects off after a week of inactivity, so the field test should use the $49/month plan).
