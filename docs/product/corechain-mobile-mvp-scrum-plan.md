...
- **Verified in code, not on screen — same constraint as E10-6.** Typecheck, lint (including the rule above) and the full test suite pass, and the production build succeeds. The admin area needs a password sign-in that Claude does not perform, so this needs the same look from the owner as E10-6 did: open `/admin/users`, and with no tester accounts yet the "Set up your first tester" card should show three unchecked steps; creating an account should check off the first two, and clicking "Copy email and password" on the resulting credentials should check the third and make the whole card disappear, leaving a "Show setup checklist" link in its place.
