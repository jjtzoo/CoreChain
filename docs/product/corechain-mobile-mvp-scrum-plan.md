...
- **E12-4 — Standards, blanks and duplicates (8).** Not built. Needs the assay import (see the later backlog) and a laboratory view (E13).
...
**Not yet done for E13:** the database migration (`20260922075705_laboratory_assay_results` — adds `received` to `CustodyEventType`, `assay_results`, `Dispatch.resultsReturnedAt`) has not been applied to the live database yet; per CLAUDE.md, that is the owner's own step (`npx prisma migrate deploy`). Until then `/laboratory` will error against the live site. No one has signed in as a laboratory account to click through the screen yet — verified in code only (typecheck, lint, the full test suite including a new schema-agreement fix for the `received` enum value, and a production build all pass).
...
- **Migration `20260922075705_laboratory_assay_results`** (adds `received` to `CustodyEventType`, the new `assay_results` table, `Dispatch.resultsReturnedAt`) is written and applies cleanly locally, but **has not been run against the live database** — per CLAUDE.md, that is the owner's own step (`npx prisma migrate deploy`), not something done without being asked each time.
- **Verified in code only for everything built today:** typecheck, lint (both apps), the full test suite (627 tests: 228 web + 399 domain, including 11 new for `packages/domain/src/laboratory.ts` and a fix to the existing schema-agreement guard test for the new enum value), and a production build all pass. Nothing above has been clicked through signed in as a project manager or laboratory account — Claude does not enter the owner's password into any login field, so that verification is the owner's own step once the migration is live.
